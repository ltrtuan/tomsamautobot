# views/action_params/write_txt_params.py
...
class WriteTxtParams(BaseActionParams):
    """UI for Write TXT File action parameters."""
    
    def __init__(self, parent_frame, parameters=None):
        super().__init__(parent_frame, parameters)
    
    def create_params(self):
        """Create UI for write txt file parameters"""
        self.clear_frame()
    ...
    def create_content_section(self):
        ...
        label = tk.Label(
            content_frame,
            text="Content to Write (separate by ; - write all lines in order):",  # ← CẬP NHẬT
            bg=cfg.LIGHT_BG_COLOR,
            font=("Segoe UI", 10)
        )
    ...
    def browse_file(self):
        ...
        if file_path:
            ...
            print(f"[WRITE_TXT_PARAMS] Selected file: {file_path}")
